...
export const TOPICS: Topic[] = [
  ...
  /* ─────────────── 2. 도구 & 터미널 ─────────────── */
  {
    ...
    Content: () => (
      <div className="space-y-6">
        ...
        />
        <div className={grid3}>
          <InfoCard icon="🖥️" title="① Warp (워프)">
            우리의 <b>작업 공간</b>. 명령어를 치는 터미널 + 코드 에디터 + 파일 목록이 한 앱에 다 들어있어요.
          </InfoCard>
          <InfoCard icon="🤖" title="② Claude Code">
    ...
    Content: () => (
      <div className="space-y-6">
        ...
        <Callout variant="warn">
          ⌨️ <b>Plan Mode 켜는 법:</b> Mac은 <Kbd>Shift</Kbd>+<Kbd>Tab</Kbd> 두 번, Windows는 <Kbd>/plan</Kbd> 입력이
    ...
    Content: () => (
      <div className="space-y-6">
        ...
        <VS
          ...
          right={
            <VSCard tone="good" title="☁️ 서버 창고(Supabase)에 올린 사진">
        ...
        <div className={grid3}>
          <InfoCard icon="1️⃣" title="가입 + 프로젝트 생성">
            supabase.com 가입 → New Project로 무료 프로젝트 생성 (이름·비밀번호 정하기).
          </InfoCard>
          <InfoCard icon="2️⃣" title="Storage에 사진 업로드">
            왼쪽 Storage 메뉴 → 새 버킷(폴더) 만들기 → 넣고 싶은 사진 1장 업로드.
          </InfoCard>
          <InfoCard icon="3️⃣" title="키 2가지 메모">
  ...
  /* ─────────────── 6. 보안 ─────────────── */
  {
    ...
    Content: () => (
      <div className="space-y-6">
        <SectionTitle
          kicker="⚠️ Security · 꼭 알아야 해요"
          title="키에는 두 종류가 있다"
          desc="“키(key)”는 창고 문을 여는 열쇠 같은 거예요. 그런데 두 종류를 절대 헷갈리면 안 돼요!"
        />
        <div className={grid2}>
          <InfoCard icon="🚪" title="anon (공개) 키 — 안전" className="border-success/50">
            원래 <b>공개용</b>이에요. 사이트 코드에 들어가도 괜찮아요. 가게 출입문처럼 누구나 들어와 구경 OK.
          </InfoCard>
          <InfoCard icon="🔑" title="service (비밀) 키 — 위험!" className="border-destructive/50">
  ...
  /* ─────────────── 9. 하네스 엔지니어링 ─────────────── */
  {
    ...
    Content: () => (
      <div className="space-y-6">
        ...
        <VS
          left={
            <VSCard tone="bad" title="❌ '이거 만들어줘'">
              AI가 만듦 → “아닌데...” → 다시 시킴 → 시간만 날림.
            </VSCard>
          }
          right={
            <VSCard tone="good" title="✔ '환경을 깔아줄게, 그 안에서 일해'">
              AI가 <b>일관되게 좋은 결과</b>를 내요. 한 번 만들어두면 계속 편해요.
            </VSCard>
          }
        />
        <Callout variant="info">
        ...
        <SectionTitle kicker="Checklist" title="집에서 진짜로 해보기" desc="눌러서 체크하면 저장돼요. 환경 설정은 어른의 도움을 받아도 좋아요!" />
        <Checklist
          storageKey="all"
          items={[
